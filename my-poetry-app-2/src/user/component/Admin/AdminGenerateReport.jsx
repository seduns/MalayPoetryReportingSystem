...
// ==========================================
export const generatePoetryPDF = (item, type = 'FULL') => {
  // 1. DATA PREP
  const authorObj = item.authorData || item.author; 
  
  const title = item.title || "Untitled";
  const authorName = authorObj?.user?.fullName || "Unknown Author";
  const authorHandle = authorObj?.user?.username ? `@${authorObj.user.username}` : "";
  const category = item.category || "GENERAL";
  const content = item.content || "No content provided.";
  const description = item.description || "No description available.";
  const statusName = item.status?.name || "PENDING"; 
  const dateStr = item.dateCreated ? new Date(item.dateCreated).toLocaleDateString() : "N/A";

  // 2. ANALYTICS
  // We prioritize the normalized 'views'/'likes' created in filteredData
  // If those are missing, we check the raw backend fields 'viewCount'/'likeCount'
  ...
  const statusBg = isApproved ? "#16a34a" : "#f59e0b";

  // 3. SCORING
  const isPoor = Number(engageRatio) < 1;
  const rating = isPoor ? "UNSATISFACTORY" : "SATISFACTORY";
  const insight = isPoor 
    ? "High visibility with low interaction indicates the content may not be resonating."
    : "Strong engagement levels suggest high quality and reader connection.";
  const action = isPoor
    ? "Recommend reviewing content themes, tags, and title to better align with reader interests."
    : "Continue with current content strategy. Consider featuring this author in upcoming newsletters.";

  // PAGE 1: ANALYTICS DETAIL
  const analyticsPage = `
    <div style="padding: 15mm; height: 260mm; width: 100%; position: relative; box-sizing: border-box; page-break-after: always;">
       ...
       <section style="background: #f9fafb; border-left: 4px solid #1e3a8a; padding: 25px; border-radius: 0 10px 10px 0; margin-bottom: 30px;">
           ...
           <div style="white-space: pre-line; font-family: serif; font-size: 12pt; color: #374151; line-height: 1.6; font-style: italic; max-height: 400px; overflow: hidden;">
  ...
  // PAGE 2: SUMMARY
  const summaryPage = `
    <div style="padding: 15mm; height: 260mm; width: 100%; position: relative; box-sizing: border-box; display: flex; flex-direction: column;">
       ...
       <div style="flex-grow: 1;">
           ...
           <div style="display: flex; gap: 15px;">
               ...
               <p style="font-size: 10pt; color: #374151; margin: 0; line-height: 1.6;">${action}</p>
           </div>
       </div>
  ...
  const options = {
    margin: 0,
    filename: `${type}_Report_${title.replace(/\s+/g, '_')}.pdf`,
...
export default function AdminGenerateReport() {
  ...
  // ✅ FIX: Use 'poetryAnalytics' to match your slice structure
  ...
  const filteredData = useMemo(() => {
    ...
    // Ensure we have an array to search through
    const analyticsList = Array.isArray(poetryAnalytics) ? poetryAnalytics : [];

    return poetryList.map((poetry) => {
      // 1. Find Match: Using String conversion to prevent ID mismatch (e.g. 7 vs "7")
      // We look into 'ans.poetry.id' as per your backend JSON structure
      const analyticsMatch = analyticsList.find(ans => 
        ans.poetry && String(ans.poetry.id) === String(poetry.id)
      );
      ...
      return {
        ...
        // We ensure 'views' and 'likes' exist by reading from 'viewCount'/'likeCount'
        analytics: { 
            views: analyticsMatch?.viewCount || 0, 
            likes: analyticsMatch?.likeCount || 0,
            ...analyticsMatch 
        }
      };
    }).filter((item) => {
  ...
  }, [searchTerm, poetryList, poetryAnalytics]);
        ...
        <div className="overflow-y-auto">
          <table className="w-full border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr><td colSpan="4" className="py-10 text-center text-gray-400 text-sm">Loading reports...</td></tr>
              ) : paginatedRows.length > 0 ? (
                paginatedRows.map((item, index) => (
                  <tr key={item.id || index} className="hover:bg-blue-50/30 transition-colors">
                    <td className="py-2 px-8 font-bold text-gray-800 text-xs text-center">{item.title}</td>
                    <td className="py-2 px-4 font-medium text-gray-500 text-xs text-center">
                      {item.authorData?.user?.fullName || "Unknown Author"}
                    </td>
                    <td className="py-2 px-4 text-center">
                      <button 
                        onClick={() => generatePoetryPDF(item, 'FULL')} 
                    ...
                    <td className="py-2 px-8 text-center">
                      <button 
                        onClick={() => generatePoetryPDF(item, 'SUMMARY')} 
                ...
                <tr><td colSpan="4" className="py-10 text-center text-gray-400 text-sm">No results found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
