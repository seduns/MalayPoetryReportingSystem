...
export default function ContributroProfileDonation() {
    ...
    const handleWithdraw = async () => {
        const currentBalance = authorDonationData?.currentDonationBalance || 0;
        const authorId = authorDonationData?.authorId;

        if (!authorId) {
            Swal.fire("Error", "Author ID not found.", "error");
            return;
        }
        ...
        if (currentBalance <= 0) {
            Swal.fire({
                ...
                text: "You have a balance of RM 0.00 to withdraw.",
                confirmButtonColor: "#FF5C5C",
            });
            return;
        }
        ...
        const amountToWithdraw = parseFloat(amountStr);

        if (amountToWithdraw > currentBalance) {
            Swal.fire({
                icon: 'error',
                title: 'Insufficient Balance',
                text: `You cannot withdraw RM ${amountToWithdraw}. Your current balance is only RM ${currentBalance.toFixed(2)}.`,
                confirmButtonColor: "#FF5C5C",
            });
            return;
        }
        ...
        dispatch(requestWithdraw(withdrawPayload))
            .unwrap()
            .then(() => {
                Swal.fire({
                    icon: "success",
            ...
            .catch((err) => {
                Swal.fire({
                    ...
                    text: err?.message || "Something went wrong processing your request.",
                    confirmButtonColor: "#FF5C5C",
                });
            });
    };
                    ...
                    <div className="flex-1 bg-white border border-black/20 rounded-[30px] shadow-sm p-10 flex flex-col">
                        <div className="flex items-center gap-6 mb-12">
                            <div className="w-24 h-24 bg-[#FF5C5C] rounded-full flex items-center justify-center text-white text-4xl font-bold shadow-lg">
                                {userProfile.user.fullName ? userProfile.user.fullName.charAt(0).toUpperCase() : "U"}
                            </div>
                            <div>
                        ...
                        <div className="bg-[#FF5C5C] rounded-[25px] p-8 flex items-center justify-between shadow-xl shadow-red-50/50">
                            <div className="text-white">
                                ...
                                <p className="text-2xl font-bold">
                                    RM {authorDonationData.currentDonationBalance.toFixed(2)}
                                </p>
                            </div>
                            <button
                                onClick={handleWithdraw}
                                className="bg-white text-[#FF5C5C] font-bold px-10 py-3 rounded-full hover:bg-gray-50 transition-all shadow-lg active:scale-95"
                            >
                                Withdraw
                            </button>
                        </div>
